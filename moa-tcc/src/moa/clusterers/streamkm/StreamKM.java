...
public class StreamKM extends AbstractClusterer {
    ...
    public void setNumClass(int numClass){
    	numClustersOption = new IntOption(
    			"numClusters", 'k',
    			"Number of clusters to compute.", numClass);
    }
    /*feito por Elaine*/
    public void setSeed(int seed){
    	randomSeedOption = new IntOption("randomSeed", 'r',
    			"Seed for random behaviour of the classifier.", seed);	
    }
    @Override
...
public void trainOnInstanceImpl2(Instance inst) {
		...
		if ((this.numberInstances)% widthOption.getValue() == 0) {
			...
			for (int valorK=2; valorK<=(int)Math.sqrt(coresetsize)/*gtClustering.size()*/; valorK++){
				...
				//silhueta.add(new Double(silhueta(valorK,new Clustering(cleaned), ResGrupos, streamingCoreset)));
			}
		}
    }
